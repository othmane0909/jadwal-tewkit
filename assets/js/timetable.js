...
function hideLoadingOverlay() {
  var overlay = $('ut-loading-overlay');
  if (overlay) overlay.style.display = 'none';
}

function initLoadingLogo() {
  var cfg = UT_CONFIG || {};
  var wrap = $('ut-loading-logo-wrap');
  var img  = $('ut-loading-logo');
  if (!cfg.logoUrl || !wrap || !img) return;
  img.src = cfg.logoUrl;
  wrap.style.display = 'flex';
  var spinner = $('ut-loading-spinner');
...
function showMaintenance(msg) {
  ...
  var maint = $('ut-maintenance');
  if (maint) {
    var msgEl = $('ut-maintenance-msg');
    if (msgEl) msgEl.textContent = msg || 'جدول التوقيت قيد التحضير حالياً.';
...
function renderSchedule(matrix, info, totalSessions, sessions, timesMap) {
  ...
  var logoTag = cfg.logoUrl
    ? '<img src="' + esc(cfg.logoUrl) + '" class="ut-sch-logo" alt="شعار الجامعة">'
    : '';
  ...
  var logoEl = sc.querySelector('.ut-sch-logo');
  if (logoEl) {
...
function init() {
  ...
  if (cfg.siteEnabled === false) {
    showMaintenance(cfg.maintenanceMessage);
    return;
  }
